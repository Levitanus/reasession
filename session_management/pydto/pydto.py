...
class DTODescriptor:
    ...
    def __init__(
        ...
        if validator and not callable(validator):
            raise TypeError(
                "Validator for field '{}' of DTO class '{}' is not callable".
                format(field, self._dto_class_name)
            )
        self._validator = validator

        if coerce and not callable(coerce):
            raise TypeError(
                "Coerce for field '{}' of DTO class '{}' is not callable".
                format(field, self._dto_class_name)
            )
        self._coerce = coerce
    ...
    def __get__(self, instance, type):
        if not instance._initialized_dto_descriptors[self._field]:
            raise AttributeError(
                "Field '{}' of DTO class '{} is not Initialized".format(
                    self._field, self._dto_class_name
                )
            )
    ...
    def _check_value(self, value):
        if self._validator is not None and not self._validator(value):
            raise ValueError(
                "{} is not a valid value for the field '{}' or DTO class {} using its validator"
                .format(value, self._field, self._dto_class_name)
            )
    ...
    def __set__(self, instance, value):
        ...
        if self._immutable and instance._initialized_dto_descriptors[self._field
            ...
            raise AttributeError(
                "Immutable attribute '{}' of DTO class '{}' cannot be changed".
...
class DTOMeta(type):
    ...
    def __new__(cls, name, bases, class_dict, partial: bool = False):
        ...
        descriptors = {
            k: v
            for k, v in class_dict.items() if isinstance(v, tuple)
        }
        _ = [class_dict.pop(k, None) for k in descriptors]

        class_dict['__slots__'] = set(
            list(descriptors.keys()) + [
                '_dto_descriptors', '_initialized_dto_descriptors',
                '_dto_descriptors_values', '_field_validators', '_partial'
            ]
        )
        ...
        if isinstance(inst, dict):
            ...
            if not self._partial:
                ...
                for k, v in inst.items():
                    try:
                        type_checker._check_type(
                            self._dto_descriptors[k][0], v
                        )
                    except TypeError:
                        return False
            else:
                for k in self._dto_descriptors.keys():
                    try:
                        type_checker._check_type(
                            self._dto_descriptors[k][0], inst[k]
                        )
...
class DTO(metaclass=DTOMeta):
    ...
    def __str__(self):
        return '{}({})'.format(
            self.__class__.__qualname__, str(self._dto_descriptors_values)
        )
    ...
    def __eq__(self, other):
        ...
        if set(self._dto_descriptors.keys()
               ) != set(other._dto_descriptors.keys()):
            return False
